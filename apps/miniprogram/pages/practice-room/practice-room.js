...
Page({
    ...
    async loadGameState(silent = false) {
        if (!this.data.roomCode)
            return;
        try {
            const gameView = await request_1.api.get(`/practice/rooms/${this.data.roomCode}/game-state`);
    ...
    connectSocket() {
        ...
        socket_1.practiceRoomSocket.on("public_game_state", (gameState) => {
            this.setData({ gameState, connected: true });
        });
        socket_1.practiceRoomSocket.on("private_hand_state", (privateState) => {
            this.applyPrivateState(privateState);
        });
        socket_1.practiceRoomSocket.on("action_applied", (action) => {
            this.setData({ lastGameMessage: `#${action.seatNo} ${action.actionType} ${action.amount || ""}` });
        });
        socket_1.practiceRoomSocket.on("street_changed", (gameState) => {
            this.setData({ gameState, lastGameMessage: `进入 ${gameState.street}` });
        });
        socket_1.practiceRoomSocket.on("hand_finished", (gameState) => {
            this.setData({ gameState, lastGameMessage: "本手结束" });
            this.loadGameState(true);
        });
        socket_1.practiceRoomSocket.on("next_hand_started", (gameState) => {
            this.setData({ gameState, lastGameMessage: "新一手开始" });
            this.loadGameState(true);
        });
        socket_1.practiceRoomSocket.on("room_finished", (payload) => {
            if (payload?.players) {
        ...
        socket_1.practiceRoomSocket.on("invalid_action", (payload) => {
            wx.showToast({ title: payload.message || "行动不合法", icon: "none" });
        });
        socket_1.practiceRoomSocket.on("game_error", (payload) => {
        ...
        socket_1.practiceRoomSocket.getGameState(this.data.roomCode);
    },
    applyGameView(gameView, message = "", silent = false) {
    ...
    getLegalAction(actionType) {
        return this.data.legalActionMap[actionType];
    },
    ...
    async ready() {
        try {
            await request_1.api.post(`/practice/rooms/${this.data.roomCode}/ready`, { readyStatus: true });
            socket_1.practiceRoomSocket.ready(this.data.roomCode, true);
            this.loadRoom();
        }
        catch (error) {
            this.showError(error);
    ...
    async sendAction(event) {
        if (this.data.acting)
            return;
        const actionType = (event.currentTarget.dataset.action || event.target.dataset.action);
        ...
        try {
            ...
            const gameView = await request_1.api.post(`/practice/rooms/${this.data.roomCode}/actions`, payload);
    ...
    async nextHand() {
        try {
            const gameView = await request_1.api.post(`/practice/rooms/${this.data.roomCode}/next-hand`, {});
    ...
    async endRoom() {
        try {
            const room = await request_1.api.post(`/practice/rooms/${this.data.roomCode}/end`, {});
            socket_1.practiceRoomSocket.endRoom(this.data.roomCode);
            this.setData({
                room,
                gameState: null,
                privateState: null,
