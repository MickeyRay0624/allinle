...
Page({
  ...
  onUnload() {
    if (replayPollTimer !== null) clearInterval(replayPollTimer);
  ...
  startRoomPolling() {
    ...
    replayPollTimer = setInterval(async () => {
      ...
      if (!roomCode || !this.data.replay) return;
      try {
        const gameView = await api.get<any>(`/practice/rooms/${roomCode}/game-state`);
